...
def import_main(config):
    ...
    # Load all test dirs
    test_dirs = []
    f = open(os.path.join(config.testsrc, 'TEST_DIRS'))
    for line in f:
        if line.startswith('#'):
            continue
    ...
    for test_dir in test_dirs:
        f = open(os.path.join(config.testsrc, test_dir, 'TEST_FILES'))
        for line in f:
            if line.startswith('#'):
                continue
    ...
    reset_info = {}
    for test_dir in test_dirs:
        f = open(os.path.join(config.testsrc, test_dir, 'TEST_FILES_RESET'))
    ...
    for test_dir, test_input, test_index in test_inputs:
        src_test_dir = os.path.join(config.testsrc, test_dir)
        ...
        f_dst.close()
        # Get the extra paths
        test_input = TestInput(os.path.join(config.testsrc), os.path.join(test_dir, test_input))
    ...
    for path_extra in paths_extra:
        ...
        src_path_extra = os.path.join(config.testsrc, path_extra)
