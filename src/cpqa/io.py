...
class CP2KSection(object):
        ...
        while True:
            ...
            if line[0] == '&':
                if line[1:].startswith("END"):
                    ...
                    if len(check_name) > 0 and check_name != self.__name:
                        raise IOError("CP2KSection end mismatch. Expecting %s. Got %s" % (self.__name, check_name))
                    break
                else:
                    section = CP2KSection()
                    section.load(f, line)
    ...
    def load(self, f, line=None):
        ...
        try:
            ...
            raise IOError("Unexpected end of file, section '%s' not ended." % self.__name)
