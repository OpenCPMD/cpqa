    ...
    for test_input in runner.test_inputs:
        ...
        if not result.flags['ok']:
            ...
            if result.flags['verbose']:
                print >> f, ' * Test run gave some standard error.'
...
def log_html(runner, timer):
    ...
    if config.mpi_suffix is not None:
        print >> f, '<tr><th>MPI suffix</th><td>%s</td></tr>' % config.mpi_suffix
    ...
    for test_input in runner.test_inputs:
        ...
        if not result.flags['ok']:
            ...
            if result.flags['failed']:
                ...
                print >> f, '</pre>'
            if result.flags['verbose']:
                print >> f, '<p class="cat">Test run gave some standard error.</p>'
