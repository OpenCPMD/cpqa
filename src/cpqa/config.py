...
class Config(object):
    def __init__(self, args, use_last=False):
        ...
        self.ppdir = user_config.__dict__.get('ppdir', None )
        ...
        self.ppdir = string.Template(self.ppdir).safe_substitute(root=self.root, arch=self.arch, version=self.version)
